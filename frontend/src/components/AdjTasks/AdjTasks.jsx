...
const INIT = [
  { id:1, todo:"Wrong invoice amount — INV-2025-002", desc:"Maxis invoice shows 240,000 but Autocount has 280,000.", remark:"Check with Maxis AR team", src:"sales", assigned_to:"Ahmad Razif", due:"2025-02-10", status:"open", task_type:"general" },
  { id:2, todo:"Duplicate supplier invoice — SUP-2025-055", desc:"Microsoft Malaysia invoice duplicated in Feb batch.", remark:"Reference PO-2025-018", src:"pur", assigned_to:"Farah Nadia", due:"2025-02-15", status:"inprog", task_type:"general" },
  { id:3, todo:"Missing GST on DR-2025-001", desc:"Deferred revenue line posted without 6% service tax.", remark:"Coordinate with tax team", src:"sales", assigned_to:"Ahmad Razif", due:"2025-02-20", status:"done", task_type:"general" },
];
...
export default function AdjTasks({ entity = "QM", entities = [] }) {
  ...
  const [tasks, setTasks] = useState([]);
    ...
    useEffect(() => {
      if (!user) return;
      getTasks(entity || "QM", user.role, user.user_id)
  ...
  const upd = (id, s) => {
    ...
    showToast("Task #" + id + " → " + (ST_META[s]?.label || s));
  };
  ...
  const visibleTasks = isManager
    ? tasks
    : tasks.filter(t => t.assigned_to === user?.user_id || t.assigned_to === user?.display_name);

  // Manager — split into general tasks and unlock requests
  const unlockRequests = visibleTasks.filter(t => t.task_type === "unlock_request");
  const generalTasks   = visibleTasks.filter(t => t.task_type === "general");
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", flex:1, overflow:"hidden", minHeight:0 }}>
      ...
      {/* ── Manager sub-tabs: Tasks | Unlock Requests ── */}
      {isManager && (
            ...
            {filtered.map(t => {
                ...
                if (t.status === "inprog")
                  actions = (
                    <button className="btn-save" style={{ background:"#1D9E75", marginRight:4 }}
                      onClick={() => upd(t.id, "done")}>Mark Done</button>
                  );
              }
              ...
              return (
                <tr key={t.id} className="task-row">
                  ...
                  <td style={{ color:"#5f5e5a", fontSize:10 }}>{t.desc}</td>
